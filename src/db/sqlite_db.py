...
class SQLiteDB:
    ...
    def _seed_if_empty(self):
        with self._get_conn() as conn:
            if conn.execute("SELECT COUNT(*) FROM users").fetchone()[0] > 0:
                return
            users = [("alice", "ABC-1234"), ("bob", "XYZ-5678"), ("charlie", "DEF-9012"), ("diana", "GHI-3456"), ("eve", "JKL-7890"), ("frank", "MNO-1111"), ("grace", "PQR-2222"), ("henry", "STU-3333"), ("iris", "VWX-4444"), ("jack", "YZA-5555")]
            conn.executemany("INSERT OR IGNORE INTO users (nickname, plates) VALUES (?, ?)", users)
            conn.executemany("INSERT OR IGNORE INTO working_hours (id, day_of_week, open_time, close_time, description) VALUES (?, ?, ?, ?, ?)", [(1, 0, "00:00", "23:59", "24/7"), (2, 1, "08:00", "18:00", "Desk")])
            conn.executemany("INSERT OR IGNORE INTO prices (id, type, rate, unit) VALUES (?, ?, ?, ?)", [(1, "standard_hour", 5.0, "hour"), (2, "standard_day", 30.0, "day"), (3, "premium_hour", 8.0, "hour"), (4, "premium_day", 45.0, "day")])
            conn.executemany("INSERT OR REPLACE INTO availability (date, free_spaces) VALUES (?, ?)", [("2025-03-10", 50), ("2025-03-11", 45), ("2025-03-12", 30), ("2025-03-13", 20), ("2025-03-14", 60), ("2025-03-15", 80)])
            conn.commit()

    def user_exists(self, nickname: str) -> bool:
        with self._get_conn() as conn:
            return conn.execute("SELECT 1 FROM users WHERE nickname = ?", (nickname.strip(),)).fetchone() is not None

    def get_free_spaces(self, date: str) -> Optional[int]:
        with self._get_conn() as conn:
            row = conn.execute("SELECT free_spaces FROM availability WHERE date = ?", (date.strip(),)).fetchone()
    ...
    def add_reservation(self, nickname: str, date: str) -> bool:
        ...
        with self._get_conn() as conn:
            try:
                conn.execute("INSERT INTO reservations (nickname, date) VALUES (?, ?)", (nickname, date))
                conn.commit()
                return True
    ...
    def get_reservations_by_nickname(self, nickname: str) -> List[Tuple[str, ...]]:
        with self._get_conn() as conn:
            cur = conn.execute("SELECT date FROM reservations WHERE nickname = ? ORDER BY date", (nickname.strip(),))
    ...
    def get_working_hours(self) -> List[Tuple]:
        with self._get_conn() as conn:
            return conn.execute("SELECT day_of_week, open_time, close_time, description FROM working_hours").fetchall()
